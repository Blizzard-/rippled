...
/** Provides asynchronous HTTP client service on a stream.
...
class client_session
    : public asio::enable_wait_for_async <client_session <Stream>>
...
public:
    ...
    explicit client_session (Args&&... args)
        ...
        , m_strand (m_stream.get_io_service())
        , m_timer (m_stream.get_io_service())
        , m_parser (*this)
    {
    }
...
private:
    ...
    async_write_some (bool continuation = true)
    {
        ...
        if (size > 0)
        {
            m_stream.async_write_some (data, this->wrap_with_counter (
    ...
    {
        m_stream.async_read_some (boost::asio::mutable_buffers_1 (
...
public:
    ...
    /** Return the socket associated with the session. */
    /** @{ */
    socket_type&
    socket()
    {
